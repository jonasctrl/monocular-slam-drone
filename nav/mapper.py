...
class DataPoint:
    def __init__(self, pcd, tx, ty, tz, qx, qy, qz, qw):
        self.pcd = np.array(pcd)
        self.pos = np.array([tx, ty, tz])
        
        

        # self.q_mod1 = R.from_quat([0.7071, 0, 0, 0.7071])
        self.q_mod1 = R.from_quat([-0.7071, 0, 0, 0.7071])
        # self.q_mod1 = R.from_quat([0, 0, 0, 1])
...
class VoxArray:
    ...
    def plot(self):
        ...
        fig = go.Figure(data=[go.Scatter3d(x=x, y=y, z=z, mode='markers',
                                           marker=dict(size=5, color=z, colorscale='viridis'), name='obst')])
    ...
    def add_pcd(self, pcd_arr):
        for point in pcd_arr:
            coord = (point / resolution).astype(int) + self.cntr
    ...
    def add_pcd_from_datapoint(self, dpt: DataPoint):
        ...
        pcd = dpt.q_mod1.apply(pcd)
        pcd = dpt.qtr.apply(pcd)

        # translate
        pcd = pcd + dpt.pos

        c = (np.array([dpt.pos])/ resolution).astype(int) + self.cntr
        c = np.clip(c, self.bgn, self.shp-1)
        self.cam_path.append(c[0])
        

        coord = (pcd / resolution).astype(int) + self.cntr
        coord_clp = np.clip(coord, self.bgn, self.shp-1)
        # for point in pcd:
...
def parse_data(n_entries, n_skip):
    ...
    for i in range(0, min(n_entries, len(depth_df)), n_skip):
        ...
        point_cloud = depth_img_to_pcd(img, skip, fx, fy, cx, cy, factor)
...
def parse_airsim_data(n_entries, n_skip):
    ...
    # tx_lst = [v['telemetry']['position']['x'] for v in metadata_list_sorted]
    # ty_lst = [v['telemetry']['position']['y'] for v in metadata_list_sorted]
    # tz_lst = [v['telemetry']['position']['z'] for v in metadata_list_sorted]
    # qx_lst = [v['telemetry']['orientation']['x'] for v in metadata_list_sorted]
    # qy_lst = [v['telemetry']['orientation']['y'] for v in metadata_list_sorted]
    # qz_lst = [v['telemetry']['orientation']['z'] for v in metadata_list_sorted]
    # qw_lst = [v['telemetry']['orientation']['w'] for v in metadata_list_sorted]
    ...
    for i in range(0, min(n_entries, len(metadata_list_sorted)), n_skip):
        ...
        # img = cv2.imread(depth_map_full_path, cv2.IMREAD_GRAYSCALE)
        # point_cloud = cv2.reprojectImageTo3D(img, projectionMatrix)
        # print(f"type={type(point_cloud)} shape={point_cloud.shape}")
        # point_cloud = point_cloud[::skip,::skip,:]
        # print(f"type={type(point_cloud)} shape={point_cloud.shape}")
        # point_cloud = point_cloud.reshape((point_cloud.shape[0] * point_cloud.shape[1], point_cloud.shape[2]))
        # point_cloud = point_cloud.astype(np.int8)
        # print(f"type={type(point_cloud)} shape={point_cloud.shape}")
        # print(f"type={type(point_cloud)} shape={point_cloud.shape}")
        # unique_pixels = np.unique(point_cloud)
        # print(f"unique pix={unique_pixels}")
        # unique_pixels = np.unique(img)
        # print(f"unique pix={unique_pixels}")
        # plt.imshow(img, cmap='gray')  # Set cmap to 'gray' for grayscale display
        # plt.axis('off')  # Hide the axes
        # plt.show()

        # print(f"img={img.tolist()}")
        point_cloud = depth_img_to_pcd(img, skip, fx, fy, cx, cy, factor)
...
def parse_airsim_data_v2(n_entries, n_skip):
    data_dir = "data"
    dataset_dir = "data"
    # List to store the contents of all JSON files
    metadata_list = []
    ...
    cx = 320    # Principal point x
    cy = 240    # Principal point y
    # factor = 0.19583843329253367 # for the 16-bit PNG files
    factor = 1 / 0.19583843329253367 # for the 16-bit PNG files
...
vmap = VoxArray()
data_arr = parse_airsim_data_v2(img_end, img_skip)
