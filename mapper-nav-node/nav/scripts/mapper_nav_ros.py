...
from depth_anything_module import DepthAnythingEstimatorModule
from depth_anything_module_v2 import DepthAnythingEstimatorModuleV2
...
class CV2_ORB:
    def __init__(self, width=518, height=518, fov_degrees=60):
        ...
        # FLANN parameters from your example
        ...
        # Camera matrix calculation based on FOV
        focal_length = (width / 2.0) / np.tan(np.radians(fov_degrees / 2))
        cx = width / 2.0
        cy = height / 2.0
    ...
    def __quaternion_to_rotation_matrix(self, qtr):
        x, y, z, w = qtr
        return np.array([
    ...
    def process_gray(self, img, pos, qtr):
        ...
        P = self.K @ np.hstack((rot, pos.reshape(-1, 1)))

        if self.last_img is not None and des is not None and self.last_des is not None:
            ...
            try:
                ...
                if len(matches) >= 8:
                    ...
                    if F is not None:
                        ...
                        # Triangulation
                        points_4d = cv2.triangulatePoints(self.last_P, P, pts1.T, pts2.T)
                        points_3d = (points_4d[:3] / points_4d[3]).T
                        pcd = [[z, -x, -y] for (x, y, z) in points_3d]

                        # Draw matches
                        img_matches = cv2.drawMatches(self.last_img, self.last_kp, img, kp,
                                                      matches[:50], None,
